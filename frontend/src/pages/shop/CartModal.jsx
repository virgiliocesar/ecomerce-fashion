...
const CartModal = ({ products, isOpen, onClose }) => {
    return (
      ...
      ${isOpen ? "opacity-100" : "opacity-0 pointer-events-none"}`}
            ...
            ${isOpen ? 'translate-x-0' : 'transform-x-full'} `}
                ...
                <div className="p-4 mt-4">
                    <div className=" flex justify-between items-center mb-4">
                        ...
                        <button
                            ...
                            <i className="ri-xrp-fill bg-black p-1 text-white cursor-pointer hover:bg-primary"></i></button>
                    </div>
                                ...
                                products.map((item, index) => (
                                    <div key={item.id || index} className="flex flex-col md:flex-row md:items-center md:justify-between shadow-md md:p-5 p-2 mb-4">
                                        ...
                                        <div className="flex flex-row md:justify-start justify-end items-center mt-2">
                                            <button className="size-6 flex items-center justify-center px-1.5 rounded-full
                                              bg-gray-200 text-gray-700 bg-primary-cart cursor-pointer ml-8"
                                            > - </button>

                                            <span className="px-2 text-center mx-1 ">{item.quantity}</span>

                                            <button className="size-6 flex items-center justify-center px-1.5 rounded-full
                                              bg-gray-200 text-gray-700 bg-primary-cart cursor-pointer"
                                            >+</button>
                                            <div>
                                                <button className="text-red-500 hover:text-red-800 cursor-pointer ml-8">
                                                    Remover
                                                </button>
                                            </div>
                                        </div>
